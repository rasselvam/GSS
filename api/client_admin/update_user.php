...
try {
    ...
    if ($role === 'gss_admin') {
        ...
        echo json_encode(['status' => 0, 'message' => 'You cannot set this role.']);
        exit;
    }

    if ($username === '' || $firstName === '' || $lastName === '' || $phone === '' || $email === '' || $role === '') {
        http_response_code(400);
        echo json_encode(['status' => 0, 'message' => 'username, first_name, last_name, phone, email and role are required.']);
        exit;
    }
    ...
    $check = $pdo->prepare('SELECT client_id FROM Vati_Payfiller_Users WHERE user_id = ?');
    ...
    $rowClientId = isset($found['client_id']) ? (int)$found['client_id'] : 0;
