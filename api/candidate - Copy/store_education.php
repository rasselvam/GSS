...
if ($isInsufficient) {
    ...
    if (isset($_FILES['marksheet_file']['name'][$i]) && !empty($_FILES['marksheet_file']['name'][$i])) {
        error_log("Processing marksheet file for card $i");
        $marksheet_file = handleFile($_FILES['marksheet_file'], $application_id, $i, 'marksheet');
    } else {
        $old_marksheet = $_POST['old_marksheet_file'][$i] ?? '';
        if (!empty($old_marksheet) && $old_marksheet !== 'INSUFFICIENT_DOCUMENTS') {
            $marksheet_file = $old_marksheet;
            error_log("Using old marksheet file: $old_marksheet");
        }
    }

    if (isset($_FILES['degree_file']['name'][$i]) && !empty($_FILES['degree_file']['name'][$i])) {
        error_log("Processing degree file for card $i");
        $degree_file = handleFile($_FILES['degree_file'], $application_id, $i, 'degree');
    } else {
        $old_degree = $_POST['old_degree_file'][$i] ?? '';
        if (!empty($old_degree) && $old_degree !== 'INSUFFICIENT_DOCUMENTS') {
            $degree_file = $old_degree;
            error_log("Using old degree file: $old_degree");
        }
    }
}
