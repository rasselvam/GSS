document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        var base = (window.APP_BASE_URL || '').replace(/\/$/, '');
        var url = base + '/api/candidate/change_password.php';
        ...
        fetch(url, {
            ...
            .then(function (data) {
                ...
                var next = (data.data && data.data.redirect) ? String(data.data.redirect) : '';
                if (!next) next = 'modules/candidate/index.php';

                var target = resolveNextUrl(next);
                if (!target) {
                    target = resolveNextUrl('modules/candidate/index.php');
                }
