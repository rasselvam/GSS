class Success {
    ...
    static getApplicationId() {
        return document.body.dataset.applicationId || 
    ...
    static openApplicationPDF(action = 'view') {
        const appId = this.getApplicationId();
        const baseUrl = window.APP_BASE_URL
        
        if (!appId || appId === 'N/A') {
            alert('Application ID not found. Please refresh the page.');
            console.error('No application ID found');
            return;
        }
        
        console.log(`Opening application ${appId} for ${action}`);
        
        // Find the clicked button for loading state
        const button = event?.target?.closest('button') || 
                      (action === 'print' ? document.querySelector('.print-btn') : 
