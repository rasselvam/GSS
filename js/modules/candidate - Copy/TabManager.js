...
class TabManager {
    constructor(pageName, containerId, templateId, tabsId, countSelectId) {
        ...
        // Store original static methods for compatibility
        this.originalOnPageLoad = null;
    ...
    createTab(index, row = null) {
        ...
        tab.className = `${this.pageName}-tab`;
     ...
     * @param {string} selector - CSS selector for the preview container
     * @param {string} file - File name/path
     * @param {string} label - Display label
     * @param {string} folder - Upload folder (e.g., 'education', 'employment')
     * @returns {HTMLElement|null} - The preview container element
     */
    ...
    setupPreviewEvents() {
        // Only setup once
        if (window._tabManagerPreviewEventsSet) return;
        ...
        document.addEventListener('click', (e) => {
            ...
            if (previewBtn) {
                ...
                const url = previewBtn.getAttribute('data-doc-url');
                const name = previewBtn.getAttribute('data-doc-name') || 'Document';
                
                if (url) {
                    console.log(`📄 Preview requested: ${name}`, url);
                    
                    // Use global preview function
                    if (typeof window.openDocPreview === 'function') {
                        window.openDocPreview(url, name);
                    } else if (typeof window.openDocumentPreview === 'function') {
                        window.openDocumentPreview(url, name);
                    } else {
                        console.error('❌ Preview function not found');
        ...
        window._tabManagerPreviewEventsSet = true;
    }
    
    /**
     * Clear a preview (remove preview UI)
     * @param {HTMLElement} card - The card element
     * @param {string} selector - CSS selector for the preview container
     */
